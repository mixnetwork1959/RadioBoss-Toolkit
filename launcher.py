...
def confirm_cutter() -> bool:
    ...
    if not SCAN_REPORT.is_file():
        messagebox.showwarning(
            tr(lang, "scan_required"),
            tr(lang, "scan_missing"),
        )
        return False
...
class ToolkitApp(tk.Tk):
    ...
    def _build_menu(self) -> None:
        ...
        for code, label in LANGUAGES.items():
            language_menu.add_radiobutton(label=label, value=code,
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(header, text=tr(self.language, "tagline"),
        ...
        cards = [
            (tr(self.language, "scheduler"), tr(self.language, "scheduler_desc"), "scheduler"),
        ...
        footer = ttk.Label(self, text=f"Version {VERSION}  •  {tr(self.language, 'portable')}",
                           style="Sub.TLabel")
        footer.pack(pady=(12, 18))

    def _card(self, parent: ttk.Frame, row: int, column: int,
              title: str, description: str, key: str) -> None:
        ...
        ttk.Label(card, text=description, style="CardText.TLabel",
                  wraplength=270, justify="left").pack(anchor="w", pady=(8, 18))
        ttk.Button(card, text=tr(self.language, "open"), style="Tool.TButton",
                   command=lambda: launch_program(key)).pack(anchor="w", side="bottom")
